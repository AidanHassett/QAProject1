...
public class ItemControllerTest {
	...
	public void testCreate() {
		...
		Mockito.when(this.utils.getString()).thenReturn(TITLE);
		Mockito.when(this.utils.getDouble()).thenReturn(PRICE);
		Mockito.when(this.dao.create(created)).thenReturn(created);

		assertEquals(created, controller.create());

		Mockito.verify(this.utils, Mockito.times(1)).getString();
		Mockito.verify(this.utils, Mockito.times(1)).getDouble();
		Mockito.verify(this.dao, Mockito.times(1)).create(created);
	}

	@Test
	public void testReadAll() {
		List<Item> items = new ArrayList<>();
		items.add(new Item(1L, "Olives", 2.00));
	...
	public void testUpdate() {
		Item updated = new Item(1L, "Olives", 2.00);
